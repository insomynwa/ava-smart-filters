<?php
/**
 * Class: Ava_Smart_Filters_Provider_WooCommerce_Archive
 * Name: WooCommerce Archive (Ava Woo Builder)
 */
...
if ( ! class_exists( 'Ava_Smart_Filters_Provider_WooCommerce_Archive' ) ) {

	/**
	 * Define Ava_Smart_Filters_Provider_WooCommerce_Archive class
	 */
	class Ava_Smart_Filters_Provider_WooCommerce_Archive extends Ava_Smart_Filters_Provider_Base {
		...
		public function wc_loop_props() {
			return apply_filters( 'ava-smart-filters/providers/' . $this->get_id() . '/wc-loop-props', array(
				'columns',
				'name',
				'is_shortcode',
				'is_paginated',
		...
		public function set_loop_props() {
			...
			foreach ( $props as $prop ) {
				ava_smart_filters()->query->add_prop( $this->get_id(), $prop, wc_get_loop_prop( $prop ) );
			}
		...
		public function get_name() {
			return __( 'WooCommerce Archive (by AvaWooBuilder)', 'ava-smart-filters' );
		}
		...
		public function ajax_get_content() {
			...
			$query_props = ava_smart_filters()->query->get_current_query_props();

			foreach ( $booleans as $bool_prop ) {
				if ( isset( $query_props[ $bool_prop ] ) ) {
					ava_smart_filters()->query->add_prop(
						$this->get_id(),
						$bool_prop,
			...
			if ( ! class_exists( 'Elementor\Ava_Woo_Builder_Base' ) ) {
				require_once ava_woo_builder()->plugin_path(
					'includes/base/class-ava-woo-builder-base.php'
				);
			}

			if ( ! class_exists( 'Elementor\Ava_Woo_Builder_Products_Loop' ) ) {
				require_once ava_woo_builder()->plugin_path(
					'includes/widgets/shop/ava-woo-builder-products-loop.php'
				);
			}

			do_action( 'ava-smart-filters/providers/woocommerce-archive/before-ajax-content' );

			add_action( 'woocommerce_before_shop_loop', array( $this, 'add_loop_data' ), 0 );

			Elementor\Ava_Woo_Builder_Products_Loop::products_loop();

			remove_action( 'woocommerce_before_shop_loop', array( $this, 'add_loop_data' ), 0 );

			do_action( 'ava-smart-filters/providers/woocommerce-archive/after-ajax-content' );
		...
		public function get_wrapper_selector() {
			return '.elementor-ava-woo-builder-products-loop';
		}
