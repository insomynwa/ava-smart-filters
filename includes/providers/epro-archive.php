...
if ( ! class_exists( 'Ava_Smart_Filters_Provider_EPro_Archive' ) ) {

	/**
	 * Define Ava_Smart_Filters_Provider_EPro_Archive class
	 */
	class Ava_Smart_Filters_Provider_EPro_Archive extends Ava_Smart_Filters_Provider_Base {
		...
		public function store_default_query( $query ) {
			...
			$query['ava_smart_filters'] = ava_smart_filters()->query->encode_provider_data( $this->get_id() );
		...
		public function add_query_args( $query ) {
			...
			$query['ava_smart_filters'] = $this->get_id() . '/default';
