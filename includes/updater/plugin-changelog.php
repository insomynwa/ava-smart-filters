 ...
 * @since 1.0.0
 */
class Ava_Smart_Filters_Plugin_Changelog {
	...
	public function plugin_row_meta( $plugin_meta, $plugin_file ) {
		...
		if ( $plugin === $plugin_file ) {
			...
			$plugin_meta['view-details'] = sprintf( '<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
				...
				esc_attr( sprintf( __( 'More information about %s', 'ava-smart-filters' ), $this->api['name'] ) ),
				esc_attr( $this->api['name'] ),
				esc_html__( 'View details', 'ava-smart-filters' )
			);
...
if ( ! function_exists( 'ava_smart_filters_plugin_changelog' ) ) {
	function ava_smart_filters_plugin_changelog() {
		return new Ava_Smart_Filters_Plugin_Changelog();
	}
}
